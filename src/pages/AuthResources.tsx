  ...
  const handleGoogleSignIn = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}${validateRedirectPath(null, '/resources')}`
        }
      });
      if (error) throw error;
    } catch (error: any) {
      ...
      <Card className="w-full max-w-lg shadow-2xl bg-gradient-to-br from-card to-card/95 border-primary/20">
        <CardHeader className="text-center space-y-4 pb-8">
          ...
          <CardTitle className="text-3xl font-bold bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
            {isLogin ? 'Access Premium Resources' : 'Unlock Learning Materials'}
          </CardTitle>
          <CardDescription className="text-lg">
            {isLogin 
              ? 'Sign in to access our complete library of study resources and materials' 
              : 'Create an account to unlock thousands of past papers, mark schemes, and study guides'
            }
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6 px-8 pb-8">
          ...
          <div className="relative">
            ...
            <button 
              className="button x w-full relative overflow-hidden group"
          ...
          <div className="text-center pt-4">
            <p className="text-sm text-muted-foreground">
              Secure access to premium educational resources
            </p>
          </div>
        </CardContent>
      </Card>
