...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      ...
      if (session) {
        // Get the intended destination from URL params - support both redirectTo and redirect
        const urlParams = new URLSearchParams(window.location.search);
        const redirectParam = urlParams.get('redirectTo') || urlParams.get('redirect');
        const redirectTo = validateRedirectPath(redirectParam, '/practice');
        navigate(redirectTo);
      }
    };
    checkUser();
    ...
    try {
      if (isLogin) {
        ...
        const {
          ...
          password,
          options: {
            emailRedirectTo: `${window.location.origin}${validateRedirectPath(new URLSearchParams(window.location.search).get('redirectTo') || new URLSearchParams(window.location.search).get('redirect'), '/practice')}`
          }
        });
        if (error) throw error;
        toast({
  ...
  const handleGoogleSignIn = async () => {
    try {
      const {
        ...
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}${validateRedirectPath(new URLSearchParams(window.location.search).get('redirectTo') || new URLSearchParams(window.location.search).get('redirect'), '/practice')}`
        }
      });
      if (error) throw error;
    } catch (error: any) {
