
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';

export const DiscordButton = () => {
  return (
    <>
      <style>{`
        .discord-custom-button {
          display: flex;
          align-items: center;
          justify-content: center;
          gap: 10px;
          padding: 0 10px;
          color: white;
          text-shadow: 2px 2px #000;
          text-transform: uppercase;
          border: solid 2px #fff;
          letter-spacing: 1px;
          font-weight: 600;
          font-size: 17px;
          background-color: rgba(88, 101, 242, 1);
          border-radius: 50px;
          position: relative;
          overflow: hidden;
          transition: all .5s ease;
          cursor: pointer;
          text-decoration: none;
        }

        .discord-custom-button:active {
          transform: scale(.9);
          transition: all 100ms ease;
        }

        .discord-custom-button svg {
          transition: all .5s ease;
          z-index: 2;
        }

        .play {
          transition: all .5s ease;
          transition-delay: 300ms;
        }

        .discord-custom-button:hover svg {
          transform: scale(1.3) translate(-40%);
        }

        .now {
          position: absolute;
          left: 0;
          transform: translateX(290%);
          transition: all .5s ease;
          z-index: 2;
        }

        .discord-custom-button:hover .now {
          transform: translateX(90%);
          transition-delay: 300ms;
        }

        .discord-custom-button:hover .play {
          transform: translateX(200%);
          transition-delay: 300ms;
        }
      `}</style>
      
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <a
              href="https://discord.gg/xRqB4BWCcJ"
              target="_blank"
              rel="noopener noreferrer"
              className="discord-custom-button"
            >
              <svg viewBox="0 0 16 16" className="bi bi-discord" fill="currentColor" height="36" width="36" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.545 2.907a13.227 13.227 0 0 0-3.257-1.011.05.05 0 0 0-.052.025c-.141.25-.297.577-.406.833a12.19 12.19 0 0 0-3.658 0 8.258 8.258 0 0 0-.412-.833.051.051 0 0 0-.052-.025c-1.125.194-2.22.534-3.257 1.011a.041.041 0 0 0-.021.018C.356 6.024-.213 9.047.066 12.032c.001.014.01.028.021.037a13.276 13.276 0 0 0 3.995 2.02.05.05 0 0 0 .056-.019c.308-.42.582-.863.818-1.329a.05.05 0 0 0-.01-.059.051.051 0 0 0-.018-.011 8.875 8.875 0 0 1-1.248-.595.05.05 0 0 1-.02-.066.051.051 0 0 1 .015-.019c.084-.063.168-.129.248-.195a.05.05 0 0 1 .051-.007c2.619 1.196 5.454 1.196 8.041 0a.052.052 0 0 1 .053.007c.08.066.164.132.248.195a.051.051 0 0 1-.004.085 8.254 8.254 0 0 1-1.249.594.05.05 0 0 0-.03.03.052.052 0 0 0 .003.041c.24.465.515.909.817 1.329a.05.05 0 0 0 .056.019 13.235 13.235 0 0 0 4.001-2.02.049.049 0 0 0 .021-.037c.334-3.451-.559-6.449-2.366-9.106a.034.034 0 0 0-.02-.019Zm-8.198 7.307c-.789 0-1.438-.724-1.438-1.612 0-.889.637-1.613 1.438-1.613.807 0 1.45.73 1.438 1.613 0 .888-.637 1.612-1.438 1.612Zm5.316 0c-.788 0-1.438-.724-1.438-1.612 0-.889.637-1.613 1.438-1.613.807 0 1.451.73 1.438 1.613 0 .888-.631 1.612-1.438 1.612Z"></path>
              </svg>
              <span className="now">now!</span>
              <span className="play">Join</span>
            </a>
          </TooltipTrigger>
          <TooltipContent>
            <p>Join the EverythingEnglish Discord Community and learn with everyone :)</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </>
  );
};
